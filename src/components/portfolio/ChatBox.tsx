...
import { setItem, getItem } from '../../utils/common/localStorage';
...
export default function ChatBox() {
  ...
  const handleRegisterSuccess = (user: User) => {
    ...
    if (user.otpSent) {
      ...
      showSnackbar(
        `Welcome back! ${registeredUser?.displayName}, You have successfully registered.`,
        {
          severity: 'success',
  ...
  const handleOtpValidationSuccess = (status: boolean) => {
    ...
    if (status) {
      ...
      showSnackbar(
        `Congratulations! ${registeredUser?.displayName}, You have successfully registered.`,
        {
          severity: 'success',
          showCloseButton: true,
          duration: 3000,
  ...
  const visibleMessages = showOnlyLatest ? messages.slice(-1) : messages;

  return (
    <Box display="flex" justifyContent="center" width="100%" px={{ xs: 2, sm: 3, md: 6 }} py={4}>
      <Box
        width="100%"
        maxWidth={{
          xs: '100%',
      ...
      >
        ...
        <Stack direction="row" justifyContent="flex-end" spacing={2} mb={1}>
          <Button variant="text" size="small" onClick={() => setRegisterModalOpen(true)}>
            Register
          </Button>
        ...
        <ChatInput
          onSend={handleSend}
          disabled={loading}
          promptsLeft={registeredUser?.promptsLeft || 0}
        />
      </Box>
    </Box>
  );
