...
export default function ChatMessage({ id, question, answer, action, user }: Props) {
  ...
  const handleBasicActions = async (action: 'like' | 'dislike' | 'report') => {
    try {
      const response = await axios.patch(`${BACKEND_URL}/prompts/action/${id}`, { action: action });
    ...
    <Box mb={4}>
      ...
      <Box display="flex" justifyContent="flex-end">
        ...
        >
          ...
          <Typography variant="subtitle2" color="textSecondary" mb={0.5} sx={{ px: 2, pt: 1.5 }}>
            Anzo
          </Typography>
      ...
      <Box display="flex" justifyContent="flex-end" mt={1} pr={{ xs: '5%', md: '20%' }}>
        <Stack direction="row" spacing={1}>
          ...
          <IconButton
            size="small"
            aria-label="Dislike"
            onClick={() => handleBasicActions('dislike')}
          >
          ...
          {expanded && (
            <Tooltip title="Shrink the answer bubble" arrow>
              <IconButton size="small" aria-label="Collapse" onClick={() => setExpanded(false)}>
